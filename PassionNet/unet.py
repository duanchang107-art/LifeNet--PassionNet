...
from nets.unetcfc import Unet as unet
...
class Unet(object):
    _defaults = {
        ...
        #-------------------------------------------------------------------#
        "model_path"    : "logs/0.5-0.3-25loss_2026_04_04_19_56_59/best_epoch_weights.pth",
        #--------------------------------#
        #   所需要区分的类的个数+1
        #--------------------------------#
        "num_classes"   : 4,
    ...
    def generate(self, onnx=False):
        # self.net = unet(num_classes = self.num_classes, backbone=self.backbone,deep_supervision=True)

        self.net = unet(num_classes = self.num_classes, backbone=self.backbone)
        #self.net = BFnet(num_classes = self.num_classes, backbone=self.backbone)
    ...
    def get_miou_png(self, image):
        ...
        with torch.no_grad():
            ...
            #feat1=self.net(images)[2][0]
            #feat2=self.net(images)[3][0]
            #---------------------------------------------------#
            #   取出每一个像素点的种类
            #---------------------------------------------------#
            pr = F.softmax(pr.permute(1,2,0),dim = -1).cpu().numpy()
