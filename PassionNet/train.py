...
if __name__ == "__main__":
    ...
    #   数据集路径
    #------------------------------#
    VOCdevkit_path  = "/data1/dsy/WSY/pointretouch2/"
    #------------------------------------------------------------------#
    #   建议选项：
    #   种类少（几类）时，设置为True
    ...
    cls_weights     =np.array([0.05,20,20,20], dtype=np.float32)
    ...
    if Cuda:
        if distributed:
            ...
            ema_model = ema_model.cuda(local_rank)
    ...
    with open(os.path.join(VOCdevkit_path, "2.txt"), "r") as f:
          val_lines = f.readlines()
    # with open(os.path.join(VOCdevkit_path, "train.txt"),"r") as f:
    #     train_lines = f.readlines()
    # with open(os.path.join(VOCdevkit_path, "val.txt"),"r") as f:
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, 
                    epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, dice_loss, focal_loss, cls_weights,cls_weights2,num_classes, fp16, scaler, save_period, save_dir, local_rank)
