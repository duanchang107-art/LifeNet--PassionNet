...
def fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, dice_loss, focal_loss, cls_weights, num_classes, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    model_train.eval()
    # fit_one_epoch 里 forward 实际用的是 model（而不是 model_train），
    # 所以验证时也需要把 model 切到 eval，并且禁用梯度，避免显存持续增长。
    model.eval()
    # if cuda:
    ...
    for iteration, batch in enumerate(gen_val):
        ...
        with torch.no_grad():
            ...
            with amp_ctx:
                ...
                # 下面两个 cat 在验证里没有被使用，会徒增峰值显存占用，直接移除。
                # output_seg = torch.cat([output_seg_1, output_seg_2], dim=0)
                # output_cls = torch.cat([output_cls_1, output_cls_2], dim=0)
            ...
            # 及时释放引用，降低峰值/碎片化风险
            del input_var, input_var_1, input_var_2
            del output_seg_1, output_seg_2, output_cls_1, output_cls_2
            del cls_logits_1, cls_logits_2, seg_logits_1, seg_logits_2
            del target_cls, target_var_seg, target_var_cls, target_var_label
