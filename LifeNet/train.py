...
if __name__ == "__main__":
    ...
    #   数据集路径
    #------------------------------#
    # VOCdevkit_path  ='/data1/dsy/WSY/amd_countour/'
    ...
    #------------------------------------------------------------------#
    cls_weights     = np.array([0.05, 60,60,60], np.float32)
    # cls_weights     = np.array([0.05, 30,30,30], np.float32)
    #------------------------------------------------------------------#
    ...
    # 将权重文件也保存到本次训练的 log_dir 下（与 TensorBoard、曲线文件同目录）。
    # 仅 local_rank==0 会实际保存权重；其余 rank 传入占位目录不影响。
    ...
    if local_rank == 0:
        show_config(
            ...
            save_period = save_period, save_dir = run_save_dir, num_workers = num_workers, num_train = num_train, num_val = num_val
        )
    #------------------------------------------------------#
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, 
                    epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, dice_loss, focal_loss, cls_weights, num_classes, fp16, scaler, save_period, run_save_dir, local_rank)
